...
namespace ThinkDb.Dialect
{
    public abstract class SqlProvider : ISqlProvider
    {
        ...
        public virtual SqlStatement GetLiteral(SpecialExpressionType operationType, IList<SqlStatement> p)
        {
            switch (operationType) // SETuse
            {
                ...
                case SpecialExpressionType.Concat:
                    //if (p.Count > 2)
                    return GetLiteralStringConcat(p);
                ...
                case SpecialExpressionType.Avg:
        ...
        protected virtual SqlStatement GetLiteralStringConcat(SqlStatement a, SqlStatement b)
        {
            ...
            //return SqlStatement.Format("{0} || {1}", a, b);
            return GetLiteralStringConcat(new SqlStatement[] { a, b });
        }
        /// <summary>
        /// Gets the literal string concat.
        /// </summary>   
        protected virtual SqlStatement GetLiteralStringConcat(IList<SqlStatement> array)
        {
            StringBuilder format = new StringBuilder("{0}");
            for (int i = 1; i < array.Count; i++) {
                format.Append(" || ").Append("{").Append(i).Append("}");
            }
            return SqlStatement.Format(format.ToString(), array);
        }
        ...
        /// Gets the literal like.
        /// </summary>
        protected virtual SqlStatement GetLiteralLike(IList<SqlStatement> array)
        {
            return SqlStatement.Format("{0} LIKE {1}", array[0], GetLiteralStringConcat(array.Skip(1).ToList()));
        }
        ...
        /// <param name="a">A.</param>
        /// <returns></returns>
        protected virtual SqlStatement GetLiteralCount(SqlStatement a, SqlStatement b)
        {
            return SqlStatement.Format("COUNT({0}) {1}", a, b);
        }
        ...
        protected virtual SqlStatement GetLiteralIn(IList<SqlStatement> array)
        {
            return GetLiteralIn(array[0], GetLiteral(array.Skip(1).ToArray()));
        }
        ...
        protected abstract char SafeNameStartQuote { get; }
        /// <summary>
        /// Gets the safe name end quote.
        /// </summary>
        /// <value>The safe name end quote.</value>
        protected abstract char SafeNameEndQuote { get; }
        ...
        public virtual SqlStatement GetColumnAsAlias(string column, string alias)
        {
            return SqlStatement.Format("{0} {1}", column, alias);
        }
    }
}
