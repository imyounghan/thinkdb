...
using System.Text;
using ThinkDb.Sql;
//using ThinkDb.Sugar.Expressions;
...
namespace ThinkDb.Dialect
{
    public class SqlServerSqlProvider : SqlProvider
    {
        ...
        {
            return string.Concat("'", literal.ToString("o").Substring(0, 23), "'");
        }
        ...
        protected override SqlStatement GetLiteralStringConcat(IList<SqlStatement> array)
        {
            StringBuilder format = new StringBuilder("{0}");
            for (int i = 1; i < array.Count; i++) {
                format.Append(" + ").Append("{").Append(i).Append("}");
            }
            var list = array.Select(item => item.Replace("sql_variant", "varchar", false)).ToList();
            return SqlStatement.Format(format.ToString(), list);
        }
        ...
        public override SqlStatement GetColumnAsAlias(string column, string alias)
        {
            return SqlStatement.Format("{0} AS {1}", column, alias);
        }
    }
}
